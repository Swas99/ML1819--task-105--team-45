...
int main(void) 
{ 
    string input1 = "haberman.txt";
    string input2 = "haberman.txt";
    string ds_clases = 
    ...
    string ns_ds1 = 
    "C:/Users/Swastik/Desktop/MastersDegree_CS/Semester_1/MachineLearning/GroupAssingment/noisyDataSets/ML_Algorithms/data_set/linear_regression/ds_2/ns_ds_2/";
    ...
    for(int i =0; i<SUBSET_SIZE && i<lineNumber; i++)
    {
        ...
        int sign = dist9(rng);
        if(sign<5)
            originalDataSet[i].y = (100+n) * originalDataSet[i].y/100.0;
        else
            originalDataSet[i].y = (100-n) * originalDataSet[i].y/100.0;
