# -*- coding: utf-8 -*-
"""
Created on Fri Oct 26 02:01:50 2018
...
def computeCost(X, y, theta):
  diff=np.subtract(predict(X,theta),y)
  squ=np.square(diff)
  sum=np.sum(squ)
  count=np.prod(squ.shape)
  cost = (1/(2*count))*sum
  return cost

def computeGradient(X,y,theta):
  difff=np.subtract(predict(X,theta),y)
  comp=difff.dot(X)
  grad=(1/np.prod(difff.shape))*comp
  return grad
